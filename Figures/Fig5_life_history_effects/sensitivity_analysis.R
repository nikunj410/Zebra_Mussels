...
de <- function(v,w,B22,B33,B44) return(t(v) %*% diag(c(0,B22,B33,B44)) %*% w/sum(v*w)) 

arrival_time<- function(F2,F3,F4,S1,S2,S3,S4, B22, B33, B44, gm_Aij){
  Leslie = Leslie_matrix(F2,F3,F4,S1,S2,S3,S4)
  v = reproductive.value(Leslie)
  w = stable.stage(Leslie)
  lambda = lambda(Leslie)
  de = de(v,w,B22,B33,B44)
...
elasticity_time = as.data.frame(matrix(0, nrow  = iterations, ncol = 11))
colnames(elasticity_time) = c("F2","F3","F4","S1","S2","S3","S4", "B22", "B33", "B44", "gm_Aij")
...
for(i in 1:iterations)
{
  ...
  w = stable.stage(Leslie)
  lambda = lambda(Leslie)
  Bii = draw$deff[i]*sum(v*w)/sum(v[2:4]*w[2:4])
  B22 = Bii
  B33 = Bii
  B44 = Bii
  original = arrival_time(F2,F3,F4,S1,S2,S3,S4, B22, B33, B44, gm_Aij)
  
  elasticity_time[i,] = foreach(Npar = 1:11, .combine = 'c') %dopar%{
    -(log(original/arrival_time(Npar == 1 ? (F2*OnePlusDelta) : F2,
                                             ...
                                             Npar == 8 ? (B22*OnePlusDelta) : B22,
                                             Npar == 9 ? (B33*OnePlusDelta) : B33,
                                             Npar == 10 ? (B44*OnePlusDelta) : B44,
                                             Npar == 11 ? (gm_Aij*OnePlusDelta) : gm_Aij))/log(OnePlusDelta))
  }
}

elasticity_time_ggplot = as.data.frame(cbind(rep(c("F2","F3","F4","S1","S2","S3","S4", "B22", "B33", "B44", "gm_Aij"),iterations),as.vector(t(as.matrix(elasticity_time)))))
...
sensitivity  = ggplot(data = elasticity_time_ggplot, aes(x=reorder(var,-elasticity), y=elasticity)) +
  ...
  scale_x_discrete(labels = rev(c(expression(S[1]),expression(B[22]),
                                  expression(F[2]),expression(S[2]),
                                  expression(F[3]), expression(B[33]), expression(F[4]),
...
ggsave("Figures/Fig5_life_history_effects/sensitivity.pdf",all_plots, width = 18, height = 8, units = c("cm"))
